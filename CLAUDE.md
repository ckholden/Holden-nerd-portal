...
| `878/` + `878/codeplug/` | **878 codeplug search + download** — search the shared AnyTone 878 DMR codeplug by channel/frequency/talkgroup (`878/codeplug/index.html` reads static `878/codeplug/data.json`), plus per-person download zips (`878/codeplug/downloads/*.zip`). Data is regenerated by `878/codeplug/export.py`, which reads live from `OneDrive\radio\878\Christian KJ7DTS\` on the SAME machine (not committed source-of-truth — that CSV folder is) — **run it + commit/push after any codeplug-editing session**, see the radio project's own `CLAUDE.md` for the exact trigger rule. | Own `/878/portal-auth-878.js` Firebase gate, narrower allowlist than the portal (just Christian/KJ7DTS + Chris/KK7ION + Pete/KK7RBQ) |
